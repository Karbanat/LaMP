...
app = FastAPI(title="PC-Shop.gg", description="Магазин игровых компьютеров")
...
async def startup():
    ...
    async with AsyncSession(engine) as session:
        ...
        if not count:
            products = [
                Product(
                    name="PC-Shop Light Edition",
                    price=45000,
                    description="Базовый игровой компьютер: RTX 3050, i5-12400F, 16GB RAM, 500GB SSD",
                    image_path=""
                ),
                Product(
                    name="PC-Shop Medium Edition",
                    price=75000,
                    description="Средний игровой компьютер: RTX 3060, i5-13400F, 32GB RAM, 1TB SSD",
                    image_path=""
                ),
                Product(
                    name="PC-Shop Normal Edition",
                    price=95000,
                    description="Оптимальный игровой компьютер: RTX 4060, i7-13700F, 32GB RAM, 2TB NVME",
                    image_path=""
                ),
                Product(
                    name="PC-Shop Pro Edition",
                    price=145000,
                    description="Профессиональный игровой компьютер: RTX 4070 Ti, i7-14700K, 64GB RAM, 2TB NVME",
                    image_path=""
                ),
                Product(
                    name="PC-Shop Pro-Hard Edition",
                    price=220000,
                    description="Максимальная производительность: RTX 4090, i9-14900K, 128GB RAM, 4TB NVME",
                    image_path=""
                ),
            ]
...
async def read_products(db: AsyncSession = Depends(get_db)):
    ...
    for p in products:
        if p.image_path and os.path.exists(p.image_path):
            ...
            # Статические изображения для предустановленных товаров
            if "Light" in p.name:
                p.image_url = "/static/imgs/light.png"
            elif "Medium" in p.name:
                p.image_url = "/static/imgs/medium.png"
            elif "Normal" in p.name:
                p.image_url = "/static/imgs/normal.png"
            elif "Pro Edition" in p.name:
                p.image_url = "/static/imgs/pro.png"
            elif "Pro-Hard" in p.name:
                p.image_url = "/static/imgs/pro-hard.png"
            else:
...
@app.get("/health")
async def health():
    return {"status": "ok", "shop": "PC-Shop.gg", "theme": "neon"}
